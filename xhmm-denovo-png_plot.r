...
XHMM_PATH = "./"
#XHMM_SCRIPTS_PATH = "/home/sz97/bin/scripts/XHMM/xhmm/sources/scripts/R_functions"
XHMM_SCRIPTS_PATH = "/home/jc2545/programs/statgen-xhmm-3c57d886bc96/sources/scripts/R_functions"
source(paste(XHMM_SCRIPTS_PATH, "/sourceDir.R", sep=""))
sourceDir(XHMM_SCRIPTS_PATH, trace = FALSE, recursive = TRUE)

PLOT_PATH ="."
JOB_PREFICES="./DATA"
JOB_TARGETS_TO_GENES="/home/sj483/Ref_Data/Ref_IDT/annotated_targets.refseq.loci"
#JOB_TARGETS_TO_GENES="/home/sj483/Ref_Data/Ref_MedExome/annotated_targets.refseq.loci"
SAMPLE_FEATURES = NULL
USE_XCNV_TO_PLOT="./DATA.xcnv"
PEDIGREE_FILE = "./DATA.pedinfo"
PEDIGREE_DATA = readPedigreeFile(PEDIGREE_FILE)
SAMPLE_FEATURES = pedigreeDataToBinarySampleProperties(PEDIGREE_DATA)
USE_XCNV_TO_PLOT=USE_XCNV_TO_PLOT
LIMIT_MEMORY=TRUE
PLOT_ONLY_PNG=TRUE
PLOT_READ_DEPTHS=TRUE
PLOT_PC_CORRS=FALSE
PLOT_ALL_CNVS=TRUE
INCLUDE_PEDIGREE_SAMPLES=PEDIGREE_DATA
NUM_ADD_TARGS = 2
SQ_THRESH = 60

JOBS = names(JOB_PREFICES)
if (is.null(JOBS)) {
    JOBS = paste("dataset_", as.character(1:length(JOB_PREFICES)),
        sep = "")
    names(JOB_PREFICES) = JOBS
    if (!is.null(JOB_TARGETS_TO_GENES)) {
        if (length(JOB_TARGETS_TO_GENES) != length(JOBS) ||
            !is.null(names(JOB_TARGETS_TO_GENES))) {
...
if (LIMIT_MEMORY) {
    ...
    if (PLOT_READ_DEPTHS) {
        EXCLUDE_LARGE_MATRICES = setdiff(EXCLUDE_LARGE_MATRICES,
            c("PCA_NORMALIZED"))
    }
    if (PLOT_ALL_CNVS) {
        ...
        if (!PLOT_ONLY_PNG) {
            EXCLUDE_LARGE_MATRICES = setdiff(EXCLUDE_LARGE_MATRICES,
                c("filtered_centered_RD", "PCA_NORMALIZED", "DIP_POST", "DEL_POST", "DUP_POST"))
        }
    }
...
if (PLOT_READ_DEPTHS) {
    ...
    for (job in JOBS) {
        for (i in 1:length(READ_DEPTH_PIPELINE)) {
            ...
            hist(mean.target[[job]][[rd]], breaks = 400,
                xlab = "Mean target coverage", ylab = "Frequency",
                main = paste(job, ": ", rdName, sep = ""))
        }
    }
    dev.off()
    ...
    for (job in JOBS) {
        for (i in 1:length(READ_DEPTH_PIPELINE)) {
            rd = READ_DEPTH_PIPELINE[i]
            rdName = READ_DEPTH_PIPELINE_SHORT_NAMES[i]
            hist(mean.sample[[job]][[rd]], breaks = 400,
                xlab = "Mean sample coverage", ylab = "Frequency",
                main = paste(job, ": ", rdName, sep = ""))
        }
    }
    dev.off()
...
if (PLOT_PC_CORRS) {
    ...
    for (job in JOBS) {
        ...
        for (plotPC in numPCsToPlot) {
            ...
            matplot(indsToPlot, corrsToPlot, ylim = ylim, pch = pch, type = "p", col = colors, cex = PT_EXPANSION_FACTOR, xlab = "Principal component",
                    ylab = "Absolute value of correlation to PC scores/loadings", main = paste(job, "", sep = ""))
    ...
    for (job in JOBS) {
        ...
        heatmap.2(absCorrWithRemovedPC, Rowv = NA, Colv = NA, dendrogram = "none", scale = "none", revC = FALSE, density.info = "none", trace = "none", xlab = "", main = title,
                symbreaks = FALSE, colsep = 1:ncol(absCorrWithRemovedPC), sepcolor = "white")
    }
    ...
    for (job in JOBS) {
        ...
        if (system("which convert", ignore.stdout = TRUE,
        ...
        if (nrow(XCNV_CALLS) > 0) {
            for (cnvInd in 1:nrow(XCNV_CALLS)) {
                ...
                if (!is.null(INCLUDE_PEDIGREE_SAMPLES)) {
                    ...
                    MARK_SAMPLES = union(sample, as.character(unlist(INCLUDE_PEDIGREE_SAMPLES[["pedigree"]][includeRows,
                      c("#ID", "PAT", "MAT")])))
                }
                ...
                plot_XHMM_targets(usePlotName, xhmm_data[[job]], xhmm_data[[job]][["TARGETS_TO_GENES"]],SAMPLE_FEATURES, SQ_THRESH, start_stop_inds[1], start_stop_inds[2], PLOT_ONLY_PNG = PLOT_ONLY_PNG,
...
if (PLOT_READ_DEPTHS) {
    ...
    for (job in JOBS) {
        ...
        hist(sd.target, breaks = 400, xlab = "Per-target standard deviation of PCA-normalized read depth (coverage)",
            ylab = "Frequency", main = paste(job, sep = ""))
    }
        dev.off()
}
    return(xhmm_data)
