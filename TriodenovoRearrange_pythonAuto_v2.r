## Author: Xue Zeng (Modified by Garrett Allington)
## Latest modified date: 07-25-2021
...
command1=paste("java -Xmx64g -jar /home/ga352/programs/GenomeAnalysisTK-3.7/GenomeAnalysisTK.jar -nt 16 -R /home/bioinfo/software/knightlab/genomes/hs37d5/human_g1k_v37_decoy.fasta -T SelectVariants --variant ",Input," -o Trio_",Familyno,".vcf -env -sn ",Proband_ID," -sn ",Father_ID," -sn ",Mother_ID,sep="")
system(command1,intern=F)

##Regenotype
system(paste('java -Xmx64g -jar /home/ga352/programs/GenomeAnalysisTK-3.7/GenomeAnalysisTK.jar -nt 16 -R /home/bioinfo/software/knightlab/genomes/hs37d5/human_g1k_v37_decoy.fasta -T RegenotypeVariants --variant Trio_',Familyno,'.vcf -o Trio_',Familyno,'_reGT.vcf',sep = ''),intern = F)

##Split multi-allelic sites
system(paste("bcftools norm -m-both -o Trio_",Familyno,"_reGT_step1.vcf Trio_",Familyno,"_reGT.vcf",sep = ''),intern = F)

##Left normalization
system(paste("bcftools norm -f /home/sj483/Ref_Data/Public_Files/b37/human_g1k_v37.fasta -o Trio_",Familyno,"_reGT_step2.vcf Trio_",Familyno,"_reGT_step1.vcf",sep = ""),intern = F)
...
system(paste("python /ycga-gpfs/project/ysm/lifton/xz374/Scripts/ParseTrioVCF.py ",Familyno,sep = ""),intern = F)

##annovar annotate updated VCF file
command7=paste("perl /home/ga352/programs/annovar/table_annovar.pl  --vcfinput Trio_",Familyno,"_updated.vcf /home/ga352/scratch60/humandb/ -buildver hg19 -out Trio_",Familyno," -remove -protocol refGene,genomicSuperDups,snp138,dbnsfp33a,esp6500siv2_all,1000g2015aug_all,exac03,gnomad_exome,gnomad_genome -operation g,r,f,f,f,f,f,f,f -nastring '.'",sep = "")
system(command7,intern=F)

##triodenovo v0.04
command6=paste("/home/xz374/DeNovo_PCGC1/src/triodenovo.0.04/bin/triodenovo --ped Trio_",Familyno,".ped --in_vcf Trio_",Familyno,"_updated.vcf --out Trio_",Familyno,".denovo.Bayfilter.vcf --mixed_vcf_records",sep = "")
...
##Rearrangement
system(paste("python /ycga-gpfs/project/ysm/lifton/xz374/Scripts/PrepareMerge.py ",Familyno,sep = ""),intern = F)
