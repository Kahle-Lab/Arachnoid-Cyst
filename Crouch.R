...
seurat <- readRDS("/Volumes/Seagate/crouch_data.RDS")

### Print Clusters
pdf(file = '~/Desktop/Scripts/Crouch/Graphs/Cluster_Map.pdf', width = 16, height = 9)
DimPlot(seurat, reduction = "umap", raster = FALSE) #Change raster to true to reduce resolution and file size
dev.off()

### Find Markers
Crouch.markers <- FindAllMarkers(seurat, only.pos = TRUE, min.pct = 0.70, logfc.threshold = 0.50)
saveRDS(Crouch.markers, file = "~/Desktop/Scripts/Crouch/Crouch.markers.rds")
#Crouch.markers <-readRDS(file = "~/Desktop/Scripts/Crouch/Crouch.markers.rds")
TopMarks <- Crouch.markers %>%
  group_by(cluster) 
write.table(TopMarks, file = "~/Desktop/Scripts/Crouch/TopMarks.txt")

Crouch_Top20_Markers <- Crouch.markers %>%
  group_by(cluster) %>%
  slice_max(n = 20, order_by = avg_log2FC)

write.table(Crouch_Top20_Markers, file = "~/Desktop/Scripts/Crouch/Crouch_Top20_Markers.txt")
...
saveRDS(cds_from_seurat, file = "/Volumes/Seagate/Crouch_CDS_after_learn_graph.RDS")
#cds_from_seurat <- readRDS(file = "/Volumes/Seagate/Crouch_CDS_after_learn_graph.RDS")

### Plot cluster info without trajectory
pdf(file = '~/Desktop/Scripts/Crouch/Graphs/Monocle3_Cluster_Map.pdf', width = 16, height = 9)
...
pdf(file = '~/Desktop/Scripts/Crouch/Graphs/Monocle3_Cluster_Map_With_Trajectory.pdf', width = 16, height = 9)
plot_cells(cds_from_seurat, 
           color_cells_by = 'cluster',
...
pdf(file = "~/Desktop/Scripts/Crouch/Graphs/Monocle3_Cluster_Map.pdf")
...
pdf(file = "~/Desktop/Scripts/Crouch/Graphs/Monocle3_Cluster_Map_With_Trajectory.pdf")
plot_cells(cds, color_cells_by="cluster", group_cells_by = "cluster", label_groups_by_cluster = TRUE, 
           graph_label_size = 1.5, group_label_size = 4, trajectory_graph_segment_size = 0.5)
dev.off()

### Plot pseudotime trajectory
pdf(file = "~/Desktop/Scripts/Crouch/Graphs/Monocle3_Clusters_With_Pseudotime.pdf")
...
pdf(file = "~/Desktop/Scripts/Crouch/Graphs/Cluster_Top3_Marker_Gene_Epression.pdf", width = 8, height = 14)
...
CCM <- c("CCM1", "CCM2", "CCM3", "PIK3CA", "MAP3K3")
...
MasterList <- MasterList[!duplicated(MasterList)] #There are only 4 genes in analysis, so I am just making a list of those.

### Plot by-cluster enrichment for associated genes
pdf(file="~/Desktop/Scripts/Crouch/Graphs/Cluster__Gene_Expression.pdf", height = 30, width = 16)
plot_genes_by_group(cds,
                    MasterList,
                    group_cells_by="cluster",
                    ordering_type="cluster_row_col",
...
pdf(file="~/Desktop/Scripts/Crouch/Graphs/Gene_Cluster_Map.pdf", height = 10, width = 13)
plot_cells(cds,
           genes=MasterList,
           color_cells_by = "cluster",
...
cds_subset_ML <- cds[rowData(cds)$gene_short_name %in% MasterList,]
...
pdf(file="~/Desktop/Scripts/Crouch/Graphs/Crouch_Gene_Expression_Violin.pdf", height = 15, width = 15)
plot_genes_violin(cds_subset_ML, ncol=2) +
...
saveRDS(pr_graph_test_res, file = "~/Desktop/Scripts/Crouch/Files/pr_graph_test_res.RDS")
#pr_graph_test_res <-readRDS(file = "~/Desktop/Scripts/Crouch/Files/pr_graph_test_res.RDS")
...
write.table(gene_module_df, file = "~/Desktop/Scripts/Crouch/Files/gene_modules.xlsx", append = FALSE, quote = TRUE, sep = " ",
            eol = "\n", na = "NA", dec = ".", row.names = FALSE,
            col.names = TRUE, qmethod = c("escape", "double"),
            fileEncoding = " ")
#gene_module_df <- read_excel("~/Desktop/Scripts/Crouch/Files/gene_modules.xlsx", sheet = "gene_modules")
...
GMods <- gene_module_df[,1:2]
library(reshape2)
GMods <- dcast(GMods, id~module)
GMods <- GMods %>% 
  pivot_wider(names_from = module, names_prefix = "Module_", values_from = id)
for(i in 1:ncol(GMods)){
  GMods %>% separate_rows(GMods[i])
}
write.table(GMods, file = "~/Desktop/Scripts/Crouch/Files/GMods.xlsx", append = FALSE, quote = TRUE, sep = " ",
            eol = "\n", na = "NA", dec = ".", row.names = FALSE,
            col.names = TRUE, qmethod = c("escape", "double"),
            fileEncoding = " ")
#GMods <- read_excel("~/Desktop/Scripts/Crouch/Files/gene_modules.xlsx", sheet = "Sheet1") 

### Plot cell-gene module maps
pdf(file="~/Desktop/Scripts/Crouch/Graphs/UMAP_Gene_Module_Expression.pdf", height = 20, width = 20)
plot_cells(CDSP, 
...
write.table(agg_mat_df, file = "~/Desktop/Scripts/Crouch/Files/agg_mat.txt", append = FALSE, quote = TRUE, sep = " ",
            eol = "\n", na = "NA", dec = ".", row.names = TRUE,
            col.names = TRUE, qmethod = c("escape", "double"),
            fileEncoding = "")
#agg_mat_df <- read_excel("~/Desktop/Scripts/Crouch/Files/agg_mat.xlsx", sheet = "agg_mat") 
agg_mat_df <- as.data.frame(agg_mat_df) #Only needed if re-uploading from excel
agg_mat_dfr <- agg_mat_df[,-1]
row_names <- agg_mat_df[,1]
row.names(agg_mat_dfr) <- row_names

### Create cluster and gene module heatmap
pdf(file = "~/Desktop/Scripts/Crouch/Graphs/Module_Cluster_Heatmap.pdf", width = 15, height = 10)
pheatmap::pheatmap(agg_mat_dfr, cluster_rows=FALSE, cluster_cols=FALSE,
...
library(readxl)
agg_mat_NS <- read_excel("~/Desktop/Scripts/Crouch/Files/agg_mat.xlsx", sheet = "Sheet1", col_names = TRUE)
...
pdf(file = '~/Desktop/Scripts/Crouch/Graphs/Modular_CellType_Enrichment.pdf', height=9, width=16)
...
for(i in 1:ncol(GMods)){
  AVM_pVals[nrow(AVM_pVals) + 1,] = c(phyper(length(intersect(AVM, GMods[[i]])),length(AVM),(25004-length(AVM)), ModColSums[i], lower.tail=FALSE))
}
AVM_pVals = AVM_pVals[-1,]
write.table(AVM_pVals, "~/Desktop/Scripts/Crouch/Files/AVM_pVals.txt", sep = " ")

### CCM
CCM_pVals <- data.frame(var1=c(4))
for(i in 1:ncol(GMods)){
  CCM_pVals[nrow(CCM_pVals) + 1,] = c(phyper(length(intersect(CCM, GMods[[i]])),length(CCM),(25004-length(CCM)), ModColSums[i], lower.tail=FALSE))
}
CCM_pVals = CCM_pVals[-1,]
write.table(CCM_pVals, "~/Desktop/Scripts/Crouch/Files/CCM_pVals.txt", sep = " ")

### VOGM
VOGM_pVals <- data.frame(var1=c(4))
for(i in 1:ncol(GMods)){
  VOGM_pVals[nrow(VOGM_pVals) + 1,] = c(phyper(length(intersect(VOGM, GMods[[i]])),length(VOGM),(25004-length(VOGM)), ModColSums[i], lower.tail=FALSE))
}
VOGM_pVals = VOGM_pVals[-1,]
write.table(VOGM_pVals, "~/Desktop/Scripts/Crouch/Files/VOGM_pVals.txt", sep = " ")

### pVOGM
pVOGM_pVals <- data.frame(var1=c(4))
for(i in 1:ncol(GMods)){
  pVOGM_pVals[nrow(pVOGM_pVals) + 1,] = c(phyper(length(intersect(pVOGM, GMods[[i]])),length(pVOGM),(25004-length(pVOGM)), ModColSums[i], lower.tail=FALSE))
}
pVOGM_pVals = pVOGM_pVals[-1,]
write.table(pVOGM_pVals, "~/Desktop/Scripts/Crouch/Files/pVOGM_pVals.txt", sep = " ")

### AVM_VOGM
AVM_VOGM_pVals <- data.frame(var1=c(4))
for(i in 1:ncol(GMods)){
  AVM_VOGM_pVals[nrow(AVM_VOGM_pVals) + 1,] = c(phyper(length(intersect(AVM_VOGM, GMods[[i]])),length(AVM_VOGM),(25004-length(AVM_VOGM)), ModColSums[i], lower.tail=FALSE))
}
AVM_VOGM_pVals = AVM_VOGM_pVals[-1,]
write.table(AVM_VOGM_pVals, "~/Desktop/Scripts/Crouch/Files/AVM_VOGM_pVals.txt", sep = " ")

### MMD
MMD_pVals <- data.frame(var1=c(4))
for(i in 1:ncol(GMods)){
  MMD_pVals[nrow(MMD_pVals) + 1,] = c(phyper(length(intersect(MMD, GMods[[i]])),length(MMD),(25004-length(MMD)), ModColSums[i], lower.tail=FALSE))
}
MMD_pVals = MMD_pVals[-1,]
write.table(MMD_pVals, "~/Desktop/Scripts/Crouch/Files/MMD_pVals.txt", sep = " ")
...
write.table(CHD_pVals, "~/Desktop/Scripts/Crouch/Files/CHD_pVals.txt", sep = " ")
...
write.table(Height_pVals, "~/Desktop/Scripts/Crouch/Files/Height_pVals.txt", sep = " ")

### Import the concatenated MME table
library(readxl)
MME <- read_excel( "~/Desktop/Scripts/Crouch/Files/MME.xlsx")
...
MME$Module <- factor(MME$Module, levels=c("Module_1", "Module_2", "Module_3", "Module_4", "Module_5", "Module_6",
                                          "Module_7", "Module_8", "Module_9", "Module_10", "Module_11", "Module_12",
                                          "Module_13"))
MME$Set <- factor(MME$Set, levels=c("AVM", "VOGM", "AVM_VOGM", "pVOGM", "CCM", "MMD", "CHD", "Height"))
...
pdf(file = '~/Desktop/Scripts/Crouch/Graphs/Modular_Enrichment.pdf', height=9, width=16)
ggplot(MME, aes(x=Module, y= Set)) +
  geom_tile(aes(fill = Log10P), color = "white") +
  ...
  geom_text(aes(label=ifelse(as.numeric(Log10P)>2.4, round(Log10P, digits=1), '')), color = "white") + 
...
### Load in modules
GeneModules <- read_excel("~/Desktop/Scripts/Crouch/Files/gene_modules.xlsx", sheet = "Sheet1") #This is the same as the GMods dataframe
...
for(i in 1:ncol(GeneModules)){
  ...
  write.table(tmp,
              file= file.path('~/Desktop/Scripts/Crouch/EnrichR/', paste0('Module',i,'.csv')),
              sep=',',
              row.names=FALSE)
}
...
for(i in 1:ncol(GeneModules)){
  tmp <- read.csv(file=file.path('~/Desktop/Scripts/Crouch/EnrichR/', paste0('Module',i,'.csv')))
  enriched <- enrichr(tmp[[1]], dbs)
  write.table(enriched$GO_Biological_Process_2021,
              file= file.path('~/Desktop/Scripts/Crouch/EnrichR/', paste0('BP_Module',i,'.csv')),
              sep=',',
              row.names=FALSE)
  write.table(enriched$GO_Cellular_Component_2021,
              file= file.path('~/Desktop/Scripts/Crouch/EnrichR/', paste0('CC_Module',i,'.csv')),
              sep=',',
              row.names=FALSE)
  write.table(enriched$GO_Molecular_Function_2021,
              file= file.path('~/Desktop/Scripts/Crouch/EnrichR/', paste0('MF_Module',i,'.csv')),
              sep=',',
              row.names=FALSE)
  write.table(enriched$WikiPathway_2021_Human,
              file= file.path('~/Desktop/Scripts/Crouch/EnrichR/', paste0('WP_Module',i,'.csv')),
              sep=',',
              row.names=FALSE)
}
...
for (i in 1:ncol(GeneModules)) {
  tmp <- read_csv(file= file.path('~/Desktop/Scripts/Crouch/EnrichR/', paste0('BP_Module',i,'.csv')))
...
for (i in 1:ncol(GeneModules)) {
  tmp <- read_csv(file= file.path('~/Desktop/Scripts/Crouch/EnrichR/', paste0('CC_Module',i,'.csv')))
  tmp$LOG <- with(tmp, (-log10(tmp$Adjusted.P.value)))
  tmp <- as_tibble(tmp)
  tmp %>% arrange(desc(LOG))
  tmp <- as.data.frame(tmp)
...
for (i in 1:ncol(GeneModules)) {
  tmp <- read_csv(file= file.path('~/Desktop/Scripts/Crouch/EnrichR/', paste0('MF_Module',i,'.csv')))
  tmp$LOG <- with(tmp, (-log10(tmp$Adjusted.P.value)))
  tmp <- as_tibble(tmp)
  tmp %>% arrange(desc(LOG))
  tmp <- as.data.frame(tmp)
...
for (i in 1:ncol(GeneModules)) {
  tmp <- read_csv(file= file.path('~/Desktop/Scripts/Crouch/EnrichR/', paste0('WP_Module',i,'.csv')))
  tmp$LOG <- with(tmp, (-log10(tmp$Adjusted.P.value)))
  tmp <- as_tibble(tmp)
  tmp %>% arrange(desc(LOG))
  tmp <- as.data.frame(tmp)
...
library(tidyverse)
for (i in 1:ncol(GeneModules)) {
  BP <- read_csv(file= file.path('~/Desktop/Scripts/Crouch/EnrichR/', paste0('BP_Module',i,'.csv')))
  CC <- read_csv(file= file.path('~/Desktop/Scripts/Crouch/EnrichR/', paste0('CC_Module',i,'.csv')))
  MF <- read_csv(file= file.path('~/Desktop/Scripts/Crouch/EnrichR/', paste0('MF_Module',i,'.csv')))
  WP <- read_csv(file= file.path('~/Desktop/Scripts/Crouch/EnrichR/', paste0('WP_Module',i,'.csv')))
  ...
  pdf(file = file.path('~/Desktop/Scripts/Crouch/EnrichR/Graphs/', paste0('Module_', i, '.pdf')), width = 30, height = 14)
